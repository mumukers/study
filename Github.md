...
​				第一步：git fetch 远程库别名 分支（这一步会得到一个远程库别名 / 分支的分支，可以通过切换分支来查看拉取								的文件）
​								注意：查看完之后要切换回本地库分支

​				第二步：git merge 远程库别名 / 分支（合并两个分支）

​				第三步：修改文件并提交到本地库，然后push到远程库中
...
​		要点：如果不是基于Github最新版所做的修改不能推送，必须先拉取。
​					如果进入冲突状态，则按照“分支冲突解决”即可
...
​		第二步：点击fork（相当于将仓库克隆到自己的远程库中）

​		第三步：克隆到本地库中，对文件进行修改，再提交，上传到远程库中

​		第四步：Pull requests——》New  pull  requests——》Create pull  requests——》发送附加消息

主方：

​		第五步：Pull requests——》点击请求——》Commits/Files changed(审核代码)——》Merge  pull  request（合并）
​						——》 填写本次操作日志信息——》Confirm merge